...
class VastAPIClient:
    ...
    def _build_query(self, params):
        query = defaultdict(
            dict,
            {
                ...
                "gpu_total_ram": {"gte": params["gpu_total_ram"]},
                "gpu_ram": {"gte": params["gpu_ram"]},
            },
        )
        del params["verified"]
        del params["disk_space"]
        del params["instance_type"]
        del params["gpu_total_ram"]
    ...
    def list_available_instances(
        ...
        params = dict(
            list(locals().items())[-10:]
        )  # Update whenever adding parameters. Creates a dictionary from all the parameters
        query = {"q": self._build_query(params)}
        query_str = "&".join(
            [
                f"{k}={parse.quote_plus(str(v) if isinstance(v, str) else json.dumps(v))}"
        ...
        logger.debug(f"Recieved {len(offers)} results")
        return offers

    def list_current_instances(self):
        res = requests.get(
    ...
    def _set_instance_state(
        self, instance_id: str, method: Literal["PUT", "DELETE"], state: str = None
    ) -> None:
        if method == "PUT":
            res = requests.put(
    ...
    def launch_instance(
        ...
        bid_price_per_machine: float = None,
    ) -> None:
        payload = {
            "client_id": "me",
            ...
            "runtype": (
                ...
                if use_jupyter_lab
                else "ssh_direc ssh_proxy"
            ),
            "use_jupyter_lab": use_jupyter_lab,
        }
        res = requests.put(
            f"{self.BASE_URL}/asks/{instance_id}/",
    ...
    def change_bid(self, instance_id: str, price: float) -> None:
        res = requests.put(
            f"{self.BASE_URL}/instances/bid_price/{instance_id}/",
            {"client_id": "me", "price": price},
        )
        res.raise_for_status()
    ...
    def label_instance(self, instance_id: str, label: str) -> None:
        res = requests.put(
            f"{self.BASE_URL}/instances/{instance_id}/", {"label": label}
        )
        res.raise_for_status()
    ...
    def connect_ssh(
        self,
        instance_id: str,
        use_vast_proxy: bool = True,
        port_forwarding: Tuple[int, int] = None,
    ) -> paramiko.SSHClient:
        ...
        if not client.get_transport().is_active():
            raise Exception(
                "Could not connect to the SSH server using any key from SSH agent; make sure you've added it using `ssh-add <PATH_TO_PRIVATE_KEYFILE>"
            )

        if port_forwarding is not None:
            raise NotImplementedError(
                "Port forwarding blocks the main thread and I don't know how to fix it. For now, it is disabled"
            )
    ...
    def copy(self, src: str, dst: str, client: paramiko.SSHClient = None) -> None:
        ...
        if src_host != "localhost" and dst_host != "localhost":
            raise NotImplementedError(
                "Copying between remote machines is currently unsupported. Either src or dst needs to be localhost"
            )

        if client is None:
            remote_host, remote_port = (
                src_host,
                src_port if dst_host == "localhost" else dst_host,
                dst_port,
            )
    ...
    def upload(
        self, src_path: str, dst_host: str, dst_port: int, dst_path: str
    ) -> None:
        return self.copy(
            f"localhost:22:{src_path}", f"{dst_host}:{str(dst_port)}:{dst_path}"
        )

    def download(
        self, src_path: str, dst_path: str, client: paramiko.SSHClient
    ) -> None:
        sftp = paramiko.SFTPClient.from_transport(client.get_transport())
        sftp.get(dst_path, src_path)

    def download(
        self, src_host: str, src_port: int, src_path: str, dst_path: str
    ) -> None:
        return self.copy(
            f"localhost:22:{dst_path}", f"{src_host}:{str(src_port)}:{src_path}"
        )

    def get_instance_logs(self, instance_id: str, tail: int = 1000) -> str:
        res = requests.put(
            f"{self.BASE_URL}/instances/request_logs/{instance_id}/",
            params={"api_key": self.API_KEY},
            json={"tail": tail},
        )
        res.raise_for_status()
        ...
        for _ in range(10):
            ...
            if r.status_code == 200:
                return r.text
            logger.debug(
                f"Waiting on logs for instance {instance_id} fetching from {s3_url}"
            )
            time.sleep(0.5)
